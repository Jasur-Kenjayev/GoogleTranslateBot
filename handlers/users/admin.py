...
@dp.message_handler(text= "Orqaga🔜",state=new.adss,user_id=ADMINS)
...
@dp.message_handler(user_id = ADMINS,state=new.adss,content_types=types.ContentType.ANY)
...
async def send_usd(message:
	types.Message):
		...
		photo_id = "AgACAgIAAxkBAAInymKbfmI3GGbpS6YWG2nD1ykjv7dmAAL5ujEbb2DgSKJx2uMETavpAQADAgADeAADJAQ"
		await message.answer_photo(photo_id,caption=msg)
		
@dp.message_handler(text="🔚MENU🔜",user_id=ADMINS)
async def boshmenu(message:
	types.Message):
		dels = await message.answer("🏠",reply_markup=ReplyKeyboardRemove())
		await dels.delete()
		await message.answer(f"<b>🤖Xurmatli {message.from_user.full_name} Bosh Menudasiz✅</b>",reply_markup=categoryMenu)